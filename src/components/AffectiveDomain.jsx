    ...
    <div className="print:break-inside-avoid my-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {affectiveDomain.map((item, index) => (
          <div
            key={index}
            className={`bg-white rounded-lg shadow-sm p-4 border border-gray-200 ${getRatingColor(item.rating)}`}
          >
            <div className="flex justify-between items-center">
              <h4 className="font-medium text-report-dark">{item.skill}</h4>
              <span className="text-lg font-bold text-report-blue">{item.rating}</span>
            </div>
          </div>
        ))}
      </div>
